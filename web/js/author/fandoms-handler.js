...
let pairing_container = $('.pairing-container');
...
$('#fandoms-select').on('click', '.dropdown-item:not(.empty-dropdown-item)', function() {
    ...
    createOriginSection(fandom, title, item);
...
function createPairingSection() {
    let container = pairing_container;
    container.empty();
...
// пейринги
pairing_container.on('click', 'add-pairing-button', function () {
    pairing_container.append(`<div>
